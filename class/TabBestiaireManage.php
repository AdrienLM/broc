...
class TabBestiaireManage
{
  ...
  public function add($idCompte)
  {
    ...
    $q = $this->_db->prepare('SELECT * FROM 18mmi2pj02.tabbestiaire WHERE idCompte = '.$idCompte);
    $q->execute();
    $donnees = $q->fetch(PDO::FETCH_ASSOC);
    
    if($donnees==false)  // l'idCompte n'existe pas 
    {
    $q = $this->_db->prepare('INSERT INTO 18mmi2pj02.tabbestiaire (idCompte,stick1,stick2,stick3,stick4,stick5) VALUES(:idCompte,:stick1, :stick2, :stick3, :stick4, :stick5)');
  ...
  {
    $this->_db->exec('DELETE FROM 18mmi2pj02.tabbestiaire WHERE idCompte = '. $idCompte);
  }

  public function deleteAll() // on efface idCompte de la base si il existe sinon RAS
  {
    $this->_db->exec('TRUNCATE TABLE 18mmi2pj02.tabbestiaire');
  }
  ...
  public function get($idCompte)  // en fct d'un id on recupère tous les paramètres avec la clé de la base
  {
    ...
    $q = $this->_db->query('SELECT idCompte, stick1, stick2, stick3, stick4 ,stick5 FROM 18mmi2pj02.tabbestiaire WHERE idCompte = '.$idCompte);
    $donnees = $q->fetch(PDO::FETCH_ASSOC);
...
public function getEtatSticks($idCompte)  // en fct d'un id on recupère tous les paramètres selon une clé numérique
  ...
  public function getList() // on récupère la liste complète des Bestiaire (faire dump pour la trace à l'arrivée)
  {
    ...
    $q = $this->_db->query('SELECT idCompte,stick1, stick2, stick3, stick4, stick5 FROM 18mmi2pj02.tabbestiaire ORDER BY idCompte');
  ...
  //{
   // $q = $this->_db->prepare('UPDATE 18mmi2pj02.tabbestiaire SET stick'.$stickNumber.' = :stick'.$stickNumber.WHERE idCompte = '.$idCompte');
  ...
  {
    //$this->stick=$stick;
    $q = $this->_db->prepare('UPDATE 18mmi2pj02.tabbestiaire SET stick1 = :stick1, stick2 = :stick2, stick3 = :stick3, stick4 = :stick4, stick5 = :stick5 WHERE idCompte = :idCompte');
